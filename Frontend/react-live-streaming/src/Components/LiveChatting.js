...
const LiveChatting = ({ data: {data, platform} }) => {
  ...
  // 마우스 오른쪽 클릭 이벤트 핸들러
  const handleRightClick = (event, data, index) => {
    ...
    // 여기에 원하는 로직을 추가하세요. 예를 들어, 사용자 정의 컨텍스트 메뉴를 표시할 수 있습니다.
  };
  ...
  const userBlack = () => {
    ...
    axios.post(`http://${serverIP}:${port}/broadcast/addBlackList`, null,
      {
  ...
  return (
    <div>
      <MessengerContainer ref={chatContainerRef} style={{ overflowY: 'auto', maxHeight: '100%' }}>
